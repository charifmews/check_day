defmodule CheckDay.Application do
  # See https://hexdocs.pm/elixir/Application.html
  # for more information on OTP Applications
  @moduledoc false

  use Application

  @impl true
  def start(_type, _args) do
    children = [
      CheckDayWeb.Telemetry,
      CheckDay.Repo,
      {DNSCluster, query: Application.get_env(:check_day, :dns_cluster_query) || :ignore},
      {Phoenix.PubSub, name: CheckDay.PubSub},
      {Oban, Application.fetch_env!(:check_day, Oban)},
      # Start to serve requests, typically the last entry
      CheckDayWeb.Endpoint,
      {AshAuthentication.Supervisor, [otp_app: :check_day]}
    ]

    # See https://hexdocs.pm/elixir/Supervisor.html
    # for other strategies and supported options
    opts = [strategy: :one_for_one, name: CheckDay.Supervisor]
    Supervisor.start_link(children, opts)
  end

  # Tell Phoenix to update the endpoint configuration
  # whenever the application is updated.
  @impl true
  def config_change(changed, _new, removed) do
    CheckDayWeb.Endpoint.config_change(changed, removed)
    :ok
  end
end
